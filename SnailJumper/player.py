...
class Player(pygame.sprite.Sprite):
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        i = 0
        if len(obstacles) > 1:
            if player_y - obstacles[i].get('y') <= 0:
                i += 1
            if player_y - obstacles[i].get('y'):
                input_layer.append(obstacles[i].get('y') - player_y)
                input_layer.append(obstacles[i].get('x') - player_x)
                input_layer.append(obstacles[i+1].get('y') - player_y)
                input_layer.append(obstacles[i+1].get('x') - player_x)

            input_layer = np.array(input_layer)
            input_layer = np.vstack(input_layer)
            norm = np.linalg.norm(input_layer)
            normal_array = input_layer / norm
            input_vector = np.vstack(normal_array)
